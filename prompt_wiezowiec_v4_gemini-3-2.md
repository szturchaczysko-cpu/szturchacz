...
⚠️ KRYTYCZNE — NIGDY NIE POMIJAJ ZAMÓWIEŃ ZE STATUSEM DELIVERED:
- Zamówienie z Delivered i BEZ tagu (pusta koperta, brak c#:) → ZAWSZE przydziel. SCORE = 90 (pusta koperta). Nie pomijaj, nie traktuj jako błąd.
- Zamówienie z Delivered i "Zablokowany klient" → ZAWSZE przydziel. To normalny case do obsługi. Status "Zablokowany klient" lub "Nie aktywny" NIE oznacza że masz pomijać zamówienie.
- Zamówienie z Delivered i tagiem c#:...;next=DD.MM gdzie next ≤ dzisiejsza data → ZAWSZE przydziel. Termin minął lub jest dziś = do obsługi.
- Zamówienie z Delivered i nieprawidłowym tagiem (np. #5etap, "Etap 5.1 - bez skanu", coś co NIE zaczyna się od "c#:") → traktuj jak brak tagu = pusta koperta, SCORE = 90.
Jedyny powód pominięcia zamówienia z Delivered to: prawidłowy tag c#:...;next=DD.MM gdzie next > dzisiejsza data (termin przyszły → odroczone).
...
```
OPERATORZY DE:   Germany, Austria, Schweiz/Switzerland, Liechtenstein
OPERATORZY FR:   France, Belgium/Belgique, Spain/España, Italy/Italia
OPERATORZY UKPL: wszystkie pozostałe kraje (Luxembourg, Poland, Portugal,
